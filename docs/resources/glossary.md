title: Scalaris Documentation Glossary
description:
...
#### SCA
*SCA* is the utility token for the Scalaris network. Unlike USD where the smallest denomination is $0.01, the smallest incremental unit of SCA is 0.00000001 SCA.
...
#### Scalaris DX
Scalaris DX is a [decentralized exchange](#decentralized-exchange) dApp built with the [Blocknet Protocol](#blocknet-protocol) to showcase what can be built using it.

#### Scalaris
Scalaris is a self-funded and self-governed open source project with contributors around the world building an open and collaborative ecosystem with the development of the [Blocknet Protocol](#blocknet-protocol).
...
#### Block Time
*Block time* defines the time it takes to mine a [block](#blockchain-block). In all [blockchains](#blockchain) there is an expected block time. In [Scalaris](#scalaris), the expected block time is 1 minute.
...
[input](#input) (a.k.a. [UTXO](#utxo)) are returned after making a transaction. This is always an [address](#address) you own. If you have an input that's 15 SCA used in a transaction sending 10 SCA, then the remaining 5 SCA will be returned to the change address. This is a similar concept to paying for an item in cash and receiving change back. 

#### Client
Depending on the context, *client* refers either to hardware running the Scalaris blockchain or software utilizing the Blocknet Protocol, which can be a blockchain, microservice, dApp, mobile app, website, etc.
...
#### Service Node
*Service Nodes* provide services to the Blocknet [network](#network). They are users that own 2500 [SCA](#block) and have a certain setup outside of the wallet that allows them the ability to earn rewards for these service and grants them the ability to vote on [proposals](#proposals).
